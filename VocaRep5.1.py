...
##    varScores2=[]
    while startOne:
        ...
        if varAnsware1==1:
            ...
            varRemLim=int(input('Enter new reminder limit from 1 to 100 (10 is default):\n'))
...
#    varScores2=[]
    ...
    if varCorrected:
        ...
        var_iCount=0
        for i in varWords1:
            varLastRezults.append((str(i)+' - '+str(varWords2[var_iCount])+' =attempts: '+str(varScores1[var_iCount])+', successfully: '+str(varScores2[var_iCount])))
...
def FunRandomSortDic(varWords1,varWords2,varScores1,varScores2):
    ...
    varShuffleDict=[[i for i in range(0,len(varWords2))],[i for i in range(0,len(varWords2))],[i for i in range(0,len(varWords2))],[i for i in range(0,len(varWords2))]]
...
def FuncTEST1(varScores1,varScores2,varLineNow):#############inProgress
    ...
    varC=0
    for i in range(varMaxCountWords):
        if varLineNow[i][3]<=varRemLim:##############!not i - int(varWords1.index(vAns)) varLineNow[i][0]  if varScores2[i]<=varRemLim:
            print('\n',varLineNow[i][1],':')
            uAns=input()
            if uAns=='3' or uAns=='exit' or uAns=='end':
                ...
                varLineNow[i][3]+=1
                miniFunFixLocalRez(uAns, True)#varScores2!!!!!!!!!!!!!!!!!!!right
            else:
                varMemFix=0
                while varMemFix!=1:
                    ...
                    if uAns=='3' or uAns=='exit' or uAns=='end':
                        ...
                        varMemFix=1
            varLineNow[i][2]+=1
            miniFunFixLocalRez(varLineNow[i][0])#varScores2[][]+=1#varScores1!!!!!!!!!!!!!!!!!!!!!!!wrong
        else:
            varC+=1#continue
...
##    varLineNow=list(map(list, zip(*varLineNow)))#transpon!!!?????????????????????????????????????????????????
    #################################################################################
    varLineNow=FunRandomSortDic(varWords1,varWords2,varScores1,varScores2)#shuffle dict###########################################################################
    FuncTEST1(varScores1,varScores2,varLineNow)#################################!!!!!
    #################################################################################
    varSaveProgress=open('EnWScores.txt', 'w', encoding="utf-8")
    var_iCount=0
    for i in varScores1:
        varSaveProgress.write(str(varScores1[var_iCount])+' '+str(varScores2[var_iCount])+'\n')
        var_iCount+=1
